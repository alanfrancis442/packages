...
# ElevenLabs Agents Vue SDK

Build multimodal agents with the [ElevenLabs Agents platform](https://elevenlabs.io/docs/agents-platform/overview).

An SDK library for using ElevenLabs Agents with Vue.js. If you're looking for a Node.js library for other audio APIs, please refer to the [ElevenLabs Node.js Library](https://www.npmjs.com/package/@elevenlabs/elevenlabs-js).
...
```shell
npm install @elevenlabs/vue
# or
yarn add @elevenlabs/vue
# or
pnpm install @elevenlabs/vue
```
...
Vue composable for managing WebSocket and WebRTC connections and audio usage for ElevenLabs Conversational AI.
...
```ts
<script setup>
import { useConversation } from "@elevenlabs/vue";
...
// Pass it when starting the session
...
The Vue SDK supports data residency for compliance with regional regulations. You can specify the server location when initializing the conversation:
...
`startSession` method kicks off the WebSocket or WebRTC connection and starts using the microphone to communicate with the ElevenLabs Conversational AI agent. The method accepts an options object, with the `signedUrl`, `conversationToken` or `agentId` option being required.
...
```ts
<template>
  <button @click="handleFeedback(true)" :disabled="!canSendFeedback">
    👍
  </button>
  <button @click="handleFeedback(false)" :disabled="!canSendFeedback">
    👎
  </button>
</template>
...
const { sendFeedback, canSendFeedback } = useConversation();
...
const conversation = useConversation({ volume: 0.5 });
const volume = ref(0.5);

const handleVolumeChange = () => {
  conversation.setVolume({ volume: volume.value });
};
</script>
```
...
// The composable automatically applies micMuted changes
watch(micMuted, (muted) => {
  // The composable handles this automatically when you pass micMuted as a prop
});
</script>
```

Or better, pass it as a reactive property:
...
A Vue reactive ref containing the current status of the conversation.
...
// status.value is "connected" or "disconnected"
</script>
```

##### isSpeaking

A Vue reactive ref containing the information of whether the agent is currently speaking.
This is helpful for indicating the mode in your UI.

```ts
<template>
  <div v-if="isSpeaking">Agent is speaking...</div>
</template>
...
// isSpeaking.value is a boolean
</script>
```

##### canSendFeedback

A Vue reactive ref representing whether the user can send feedback to the agent.
When false, calls to `sendFeedback` will be ignored.
This is helpful to conditionally show the feedback button in your UI.

```ts
<template>
  <button @click="sendFeedback(true)" :disabled="!canSendFeedback">
    Like
  </button>
</template>
...
const { sendFeedback, canSendFeedback } = useConversation();
// canSendFeedback.value is a boolean
</script>
```

### useScribe

Vue composable for managing real-time speech-to-text transcription with ElevenLabs Scribe Realtime v2.
...
<template>
  <div>
    <button @click="handleStart" :disabled="scribe.isConnected.value">
      Start Recording
    </button>
    <button @click="handleDisconnect" :disabled="!scribe.isConnected.value">
      Stop
    </button>

    <p v-if="scribe.partialTranscript.value">
      Live: {{ scribe.partialTranscript.value }}
    </p>

    <div>
      <p
        v-for="transcript in scribe.committedTranscripts.value"
        :key="transcript.id"
      >
        {{ transcript.text }}
      </p>
...
import { useScribe } from "@elevenlabs/vue";
...
Configure the composable with default options and callbacks:

```ts
<script setup>
import { useScribe, CommitStrategy } from "@elevenlabs/vue";
...
<template>
  <div>
    <button
      ...
      :disabled="scribe.isConnected.value"
    >
      {{ scribe.status.value === "connecting" ? "Connecting..." : "Start" }}
    </button>
    <button
      @click="scribe.disconnect"
      :disabled="!scribe.isConnected.value"
    >
      Stop
    </button>

    <div v-if="scribe.partialTranscript.value">
      <strong>Speaking:</strong> {{ scribe.partialTranscript.value }}
    </div>
...
<template>
  <div>
    ...
    <button
      ...
      :disabled="!file || scribe.isConnected.value"
    >
      Transcribe
    </button>
...
<script setup>
import { ref } from "vue";
import { useScribe, AudioFormat } from "@elevenlabs/vue";
...
- **status** - Vue ref with current connection status: `"disconnected"`, `"connecting"`, `"connected"`, `"transcribing"`, or `"error"`
- **isConnected** - Vue ref with boolean indicating if connected
- **isTranscribing** - Vue ref with boolean indicating if actively transcribing
- **partialTranscript** - Vue ref with current partial (interim) transcript
- **committedTranscripts** - Vue ref with array of completed transcript segments
- **error** - Vue ref with current error message, or null
...
// Access values using .value in script, automatically unwrapped in template
console.log(scribe.status.value); // "connected"
console.log(scribe.isConnected.value); // true
console.log(scribe.partialTranscript.value); // "hello world"
console.log(scribe.committedTranscripts.value); // [{ id: "...", text: "...", words: ..., isFinal: true }]
console.log(scribe.error.value); // null or error string
</script>
```
...
Connect to Scribe. Options provided here override composable defaults:
...
All event callbacks are optional and can be provided as composable options:
...
<template>
  <div>
    ...
    <!-- Status -->
    <div>
      Status: {{ scribe.status.value }}
      <span v-if="scribe.error.value">Error: {{ scribe.error.value }}</span>
    </div>
    ...
    <div>
      ...
      <button @click="handleClearTranscripts">Clear</button>
    </div>

    <!-- Live Transcript -->
    <div v-if="scribe.partialTranscript.value">
      <strong>Live:</strong> {{ scribe.partialTranscript.value }}
    </div>

    <!-- Committed Transcripts -->
    <div>
      <h2>Transcripts ({{ scribe.committedTranscripts.value.length }})</h2>
      <div
        v-for="t in scribe.committedTranscripts.value"
        :key="t.id"
      >
